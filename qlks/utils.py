...
def find_room(price=None, floor=None, booked=False):
    ...
    if floor:
        ...
        if prooms.__len__() != 0:
            ...
            for room in rooms:
                if room.floor.__eq__(floor) and room.booked.__eq__(booked) and room.rented.__eq__(False):
                    jrooms.append(room.id)
    else:
        jrooms = prooms
    return jrooms
...
def get_all_room():
    ...
    for room in rooms:
        if room.booked.__eq__(False) and room.rented.__eq__(False):
            jrooms.append(room.id)
    return jrooms
...
def change_booked_status(room_id):
    ...
    if room.booked and room.booked.__eq__(True):
        room.booked = False
    else:
        room.booked = True
...
def room_stats(kw=None, from_date=None, to_date=None):
    pr = db.session.query(Room.id, Room.detail_id, func.sum(ReceiptDetails.quantity*ReceiptDetails.unit_price))\
        .join(ReceiptDetails, ReceiptDetails.room_id.__eq__(Room.id), isouter=True)\
        .join(Receipt, Receipt.id.__eq__(ReceiptDetails.receipt_id)).group_by(Room.id, Room.detail_id)
    if kw:
        pr = pr.filter(Room.contains(kw))
    if from_date:
...
def room_month_stats(year):
    return db.session.query(extract('month', Receipt.day),\
           ...
           .filter(extract('year', Receipt.day) == year).group_by(extract('month', Receipt.day)).all()


def find_rent(id=None):
    info = db.session.query(Room.id, RentInfo.customer_name, RentInfo.date_rent, RentInfo.date_end, RentInfo.number_of_people, RentInfo.foreign, RoomDetail.price)\
...
def add_receipt(customer_id=None):
    ...
    if customer_id:
        for tt in find_rent(customer_id):
            ...
            name = tt[1]
            day = tt[3]
        r = Receipt(customer_id=customer_id, total=total, name=name, day=day)
        db.session.add(r)
